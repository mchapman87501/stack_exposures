#pragma once

#include <filesystem>
#include <iostream>
#include <string>
#include <string_view>
#include <vector>
#include <optional>

namespace CmdLine {
const std::filesystem::path default_outpath("stacked.tiff");

using ArgVec = std::vector<std::string>;

ArgVec arg_vector(int argc, const char *argv[]);

struct CmdOptions {
  CmdOptions(const ArgVec &args);

  bool should_exit() const { return m_exit_code.has_value(); }
  int exit_code() const { return m_exit_code.value_or(0); }

  bool align() const { return m_align; }
  const auto outpath() const { return m_outpath; }
  const auto input_images() const { return m_input_images; }

private:
std::optional<int> m_exit_code;
  std::string m_invoked_as;
  bool m_align;
  std::filesystem::path m_outpath;
  std::vector<std::filesystem::path> m_input_images;

  void show_help(std::ostream &outs, int exit_code);
  void show_error(std::string_view error_msg, int exit_code);
};

} // namespace CmdLine